...
action :configure do
  ...
  template "#{new_resource.config_dir}/#{new_resource.logging_name}.conf" do
    ...
    notifies :reload, "barito_agent_setup[#{new_resource.logging_name}]", :delayed
  end
end
...
action :systemd do
  ...
  bin = "/opt/chef/embedded/lib/ruby/gems/2.5.0/gems/fluentd-0.14.25/bin/fluentd"
  ...
  unit['Service']['ExecStart'] = "/bin/bash -lc 'PATH=$PATH:/opt/chef/embedded/bin #{bin} -c #{conf}'"
